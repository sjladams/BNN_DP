...
#################################################################################
# An example of Variational Inference BNN for classification on the mnist dataset
#################################################################################

PATH = os.getcwd() + "/examples/mnist"
PATH_DATA = PATH + "/data"

URL_OXFORD = 'https://github.com/matthewwicker/ProbabilisticSafetyforBNNs/raw/master/MNIST/concurMNIST2/MNIST_Networks'
...
def load_vogn_bnn(net, args, train_loader):
    params = pickle_load(f"{PATH_DATA}/VOGN/MNIST_{args.layers}_{args.width}")
...
def load_oxford_bnn(net, args, train_loader):
    url = f"{URL_OXFORD}/VIMODEL_MNIST_{args.layers}_{args.width}_relu.net.npz"
    resp = urllib.request.urlopen(url)
    f = BytesIO(resp.read())
    params = np.load(f, allow_pickle=True, encoding='bytes')

    if not isinstance(params, np.ndarray):
        params = params['arr_0']
    # params: [w_0, b_0, w_1, b_1, ..., w_0_std, b_0_std, w_1_std, b_1_std, ...]
    ...
    param_store = pyro.get_param_store()
    for key, value in param_store.items():
        layer_idx = int(re.findall(r'\d+', key)[0])
        if 'locs' in key:
            if 'weight' in key:
                new_value = torch.from_numpy(params[layer_idx]).moveaxis(0, 1)
            else: # bias
                new_value = torch.from_numpy(params[layer_idx + 1])
        else: # scales
            if 'weight' in key:
                new_value = torch.from_numpy(params[layer_idx + len(params) // 2]).moveaxis(0, 1)
            else: # bias
                new_value = torch.from_numpy(params[layer_idx + len(params) // 2 + 1])
...
def get_bnn(args):
    train_loader, test_loader, inp_shape, out_shape = \
        data_loaders(dataset_name=args.example, batch_size_train=128, batch_size_test=128, n_inputs=args.n_inputs,
                     shuffle=False)

    # utils.plot_grid_images(next(iter(test_loader))[0][0:3], next(iter(test_loader))[1][0:3])

    bnn = get_net(args)

    if utils.available(PATH_DATA, args) and not args.train:
        bnn, guide = utils.load(bnn, PATH_DATA, args)
    elif args.trained_by == 'oxford':
        guide = load_oxford_bnn(bnn, args, train_loader)
        args.test = True
